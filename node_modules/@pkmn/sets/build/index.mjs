...
var Sets = new class {
  ...
  exportSet(s, data) {
    ...
    if (s.moves) {
      for (let move of s.moves) {
        if (move) {
          move = (_g = (_f = data == null ? void 0 : data.moves.get(move)) == null ? void 0 : _f.name) != null ? _g : move;
          buf += "- " + exportMove(move) + "  \n";
        }
      }
    }
  ...
  canonicalize(s, data) {
    ...
    s.gender = data.gen >= 2 && s.gender !== species.gender ? s.gender : void 0;
    s.level = s.level || 100;
    let maxed = true;
    if (!s.ivs) {
    ...
    const nature = data.gen < 3 ? data.natures.get(s.nature || "serious") : void 0;
...
var Teams = new class {
  ...
  importTeams(buf, data, one) {
    ...
    if (lines.length === 1 || lines.length === 2 && !lines[1]) {
      const team2 = Teams.unpackTeam(lines[0], data);
      return team2 ? [team2] : [];
    }
    const teams = [];
    let setLine = -1;
